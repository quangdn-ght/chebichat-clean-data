...
async function monitorDatabase() {
  console.log('🔍 Monitoring regions database...\n');
  ...
  try {
    ...
    console.log('📊 REGIONS DATABASE STATUS');
    console.log('='.repeat(80));
    ...
    if (recentUpdates && recentUpdates.length > 0) {
      console.log('\n📝 Recent Updates:');
      console.log('-'.repeat(80));
      recentUpdates.forEach((region, index) => {
        const updateTime = new Date(region.updated_at).toLocaleString();
        console.log(`${index + 1}. [${region.id}] ${region.name} → ${region.name_vi}`);
