...
// System prompt for province content generation
...
Your task is to process information about a Chinese province and produce a structured JSON output with the following fields:

1. name (original Chinese name - keep as provided)
2. name_vi (Vietnamese translation/transliteration of the province name)
3. short_description (a concise 1-2 sentence summary in Chinese, capturing the essence and key characteristics of the province)
4. short_description_vi (a natural, engaging 2-3 sentence summary in Vietnamese)

The descriptions should:
- Highlight the province's geographic location within China, cultural significance, or famous features
...
/**
 * Call Qwen API to generate province content
 */
async function generateProvinceContent(provinceName, retryCount = 0) {
  try {
    console.log(`📡 Calling Qwen-Max API for "${provinceName}" (attempt ${retryCount + 1})...`);
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `Process this Chinese province: ${provinceName}\n\nProvide concise descriptions in both Chinese and Vietnamese highlighting its location and key characteristics.`
        }
      ],
    ...
    console.log(`✅ Generated content for "${provinceName}"`);
    return result;
    ...
    if (retryCount < CONFIG.maxRetries) {
      ...
      return generateProvinceContent(provinceName, retryCount + 1);
    }
 ...
 */
async function fetchProvincesFromSupabase(limit = null, offset = 0) {
...
/**
 * Update province in Supabase
 */
async function updateProvinceInSupabase(provinceId, generatedData) {
  try {
    const updateData = {
    ...
    console.log(`✅ Updated province ID ${provinceId} in database`);
    return data;
    
  } catch (error) {
    console.error(`❌ Error updating province ${provinceId}:`, error);
    throw error;
  }
}
 ...
 */
export async function processProvince(province, updateDb = false) {
  const startTime = Date.now();
  
  try {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`Processing Province ID: ${province.id}`);
    console.log(`Province Code: ${province.province_code}`);
    console.log(`Name: ${province.name}`);
    console.log(`${'='.repeat(80)}`);
    
    // Generate content using Qwen API
    const generated = await generateProvinceContent(province.name);
    ...
    if (updateDb) {
      await updateProvinceInSupabase(province.id, generated);
    ...
    console.error(`\n❌ Failed to process province ${province.id}:`, error.message);
 ...
 */
export async function processProvinces(options = {}) {
  const {
    limit = null,
    offset = 0,
    updateDb = false,
  ...
  console.log('\n🚀 Starting Province Content Generation');
  ...
  try {
    // Fetch provinces
    const provinces = await fetchProvincesFromSupabase(limit, offset);
    
    if (provinces.length === 0) {
      console.log('\n✅ No provinces to process!');
      return { results: [], summary: { total: 0, success: 0, failed: 0 } };
    }
    
    console.log(`\n📊 Will process ${provinces.length} provinces\n`);
    
    // Process provinces sequentially
    const results = [];
    let successCount = 0;
    let failedCount = 0;
    ...
    for (let i = 0; i < provinces.length; i++) {
      ...
      console.log(`\n[${i + 1}/${provinces.length}] Processing "${province.name}"...`);
      
      const result = await processProvince(province, updateDb);
      results.push(result);
...
async function saveProgress(results, successCount, failedCount) {
  ...
  const filename = `province-progress-${timestamp}.json`;
...
async function saveFinalResults(results, successCount, failedCount) {
  ...
  // Save all results
  const allResultsFile = path.join(__dirname, 'output', `provinces-all-${timestamp}.json`);
  ...
  if (successResults.length > 0) {
    const successFile = path.join(__dirname, 'output', `provinces-success-${timestamp}.json`);
  ...
  if (failedResults.length > 0) {
    const failedFile = path.join(__dirname, 'output', `provinces-failed-${timestamp}.json`);
...
function printSummary(results, successCount, failedCount) {
  ...
  if (results.length > 0) {
    ...
    console.log(`⏱️  Average Time: ${avgTime.toFixed(0)}ms per province`);
